--[========================================================================]
-- ⚡ vbnx V2 AMÉLIORE - AIMLOCK + AUTO SHOOT UNIFIÉ + AUTO GRAB GUN
-- Calculs balistiques avancés + Prédiction ultra-précise
--[========================================================================]
...
local function calculateBulletTrajectory(shooterPos, targetPos, targetVel, bulletSpeed, gravity)
	-- Calcul basé sur l'équation quadratique de mouvement projectile
	...
	if discriminant < 0 then 
		return targetPos -- Impossible à atteindre
	end
	
	-- Résoudre pour le temps de vol
	...
	-- Position prédite avec multiplicateur de lead
			...
			if hum and hum.Health > 0 and rootPart then
				...
				for _, item in ipairs(char:GetChildren()) do
					if item:IsA("Tool") then
						...
						if name == "gun" or name == "revolver" then 
							hasGun = true
						elseif name == "knife" or name:find("couteau") then 
							hasKnife = true 
						end
					end
				end
				...
				if distance < closestDistance and (hasGun or hasKnife) then
...
local function updateAimlock()
	if not (AIM_CONFIG.AIMLOCK_ENABLED or AIM_CONFIG.AUTO_SHOOT_ENABLED) then 
		return 
	end
	...
	if AIM_CONFIG.AIMLOCK_ENABLED then
	...
	-- Mode Auto Shoot : Tirer automatiquement (même calcul)
	if AIM_CONFIG.AUTO_SHOOT_ENABLED and distance < AIM_CONFIG.SHOOT_DISTANCE then
		local currentTime = tick()
		if currentTime - lastShootTime > AIM_CONFIG.SHOOT_DELAY then
...
local function autoGrabGun()
	if not AIM_CONFIG.AUTO_GRAB_ENABLED then 
		return 
	end
	
	local currentTime = tick()
	if currentTime - lastGrabTime < 0.5 then 
		return -- Anti-spam
	end
...
RunService.RenderStepped:Connect(function()
	pcall(function()
		if localPlayer.Character then
			...
			-- Mise à jour aimlock + auto shoot (même calcul de prédiction)
			updateAimlock()
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if input.KeyCode == Enum.KeyCode.G then
		AIM_CONFIG.AIMLOCK_ENABLED = not AIM_CONFIG.AIMLOCK_ENABLED
		print("🎯 Aimlock: " .. (AIM_CONFIG.AIMLOCK_ENABLED and "✅ ON" or "❌ OFF"))
	end
...
print("⌨️  Touches: [G]=Aimlock | [H]=AutoShoot | [J]=AutoGrabGun")
print("🎯 Aimlock et Auto Shoot utilisent le MÊME calcul de prédiction balistique!")
